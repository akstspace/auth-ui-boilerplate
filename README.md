...
1. **Open in Dev Container** — Open this repo in VS Code, then `Cmd+Shift+P` → "Dev Containers: Reopen in Container". This spins up an Ubuntu container with Node.js (Volta) and a PostgreSQL 18 instance automatically.

2. **Install prerequisites** — Inside the container, run:
   ```bash
   volta install node
   pyenv install 3.12
   pyenv global 3.12
...
3. **Configure env** — Copy and edit the example env file:
   ```bash
   cp .env.example .env
   ```
...
2. `cp .env.example .env` — update `DATABASE_URL` to point to your local Postgres.
...
| Command | Description |
|---|---|
| `npm run db:push` | Push schema directly to DB (dev — no migration files) |
...
Config is in `drizzle.config.ts` — it reads `DATABASE_URL` from your env and outputs migrations to `src/db/migrations/`.
...
- [dreamsofcode-io/authly](https://github.com/dreamsofcode-io/authly) — Initial set of code, from which this project was updated and extended.
